...
// Get all enrollments with details - FIXED QUERY
...
$activeEnrollments = count(array_filter($enrollments, function($e) { return $e['status'] == 'enrolled'; }));
$completedEnrollments = count(array_filter($enrollments, function($e) { return $e['status'] == 'completed'; }));
$gradedEnrollments = count(array_filter($enrollments, function($e) { return !empty($e['grade']); }));
?>
<div class="container-fluid fade-in">
    ...
    <?php if (isset($_GET['deleted'])): ?>
        <div class="alert alert-success alert-dismissible fade show">
            <i class="fas fa-check-circle me-2"></i>Enrollment deleted successfully!
            <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
        </div>
    <?php endif; ?>
    ...
    <div class="row g-4 mb-4">
        ...
        <div class="col-md-3">
            <div class="stat-card" style="border-left: 4px solid var(--primary-color);">
                <div class="stat-number"><?php echo $activeEnrollments; ?></div>
                <div class="stat-label">Active Enrollments</div>
            </div>
        </div>
        <div class="col-md-3">
            <div class="stat-card" style="border-left: 4px solid var(--success-color);">
                <div class="stat-number"><?php echo $completedEnrollments; ?></div>
                <div class="stat-label">Completed</div>
            </div>
        </div>
        <div class="col-md-3">
            <div class="stat-card" style="border-left: 4px solid var(--info-color);">
                <div class="stat-number"><?php echo $gradedEnrollments; ?></div>
                    ...
                    <tbody>
                        <?php if (!empty($enrollments)): ?>
                            ...
                            <tr>
                                ...
                                <td>
                                    <span class="badge bg-secondary">Semester <?php echo $enrollment['semester']; ?></span>
                                </td>
                                <td><?php echo $enrollment['year']; ?></td>
                                <td>
                                    <?php if (!empty($enrollment['grade'])): ?>
                                        <span class="badge bg-<?php echo $enrollment['grade'] >= 'B' ? 'success' : ($enrollment['grade'] >= 'C' ? 'warning' : 'danger'); ?>" style="font-size: 0.9rem; padding: 6px 14px;">
                                ...
                                <td>
                                    <span class="badge bg-<?php echo $enrollment['status'] == 'completed' ? 'success' : ($enrollment['status'] == 'enrolled' ? 'primary' : 'danger'); ?>" style="font-size: 0.85rem; padding: 6px 14px;">
                                        <?php echo ucfirst($enrollment['status']); ?>
                                    </span>
                                </td>
                                <td class="text-center">
                                    <button class="btn btn-sm btn-info" onclick="editEnrollment(<?php echo $enrollment['enrollment_id']; ?>, '<?php echo $enrollment['grade']; ?>', '<?php echo $enrollment['status']; ?>')" title="Edit">
                                        <i class="fas fa-edit"></i>
                                    </button>
                                    <a href="enrollments.php?delete=<?php echo $enrollment['enrollment_id']; ?>" class="btn btn-sm btn-danger" title="Delete" onclick="return confirm('Delete this enrollment permanently?')">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form method="POST">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                    <button type="submit" name="edit_enrollment" class="btn btn-warning">
